# Codex Rule Reference: distributed

Migrated from `.claude/rules/distributed.md`.
...
For any usage of `DTensor.to_local`, specify explicitly the
`grad_placements`, especially when the original DTensor placement has `Replicate`
or `Partial`. This includes the indirect calls from `local_map`
(`in_grad_placements`), and `full_tensor` (`grad_placements`).
